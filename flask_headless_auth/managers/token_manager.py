...
from flask_jwt_extended import (
    ...
    set_refresh_cookies, unset_jwt_cookies, verify_jwt_in_request, decode_token
)
...
class TokenManager:
    ...
    def generate_token_authsvc(self, user, additional_claims=None,
        ...
        # ------- Update session with real JTI from JWT (FIX for JTI mismatch) -------
        if audit_mgr:
            try:
                # Decode the refresh token to get the real JTI generated by flask-jwt-extended
        ...
        if audit_mgr:
            ...
            # Use real JTI if available, fallback to placeholder
            try:
                decoded_refresh = decode_token(refresh_token)
                final_jti = decoded_refresh.get('jti', placeholder_jti)
            except Exception:
                final_jti = placeholder_jti
            audit_mgr.enforce_single_session(user['id'], final_jti)
